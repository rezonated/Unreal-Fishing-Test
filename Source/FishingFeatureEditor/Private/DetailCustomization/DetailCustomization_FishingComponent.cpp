...
void FDetailCustomization_FishingComponent::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
    ...
    OwnerSkeletalMeshProperty->SetOnPropertyValueChanged(/*FSimpleDelegate::CreateSP(this, &FDetailCustomization_FishingComponent::OnSkeletalMeshChanged, OwnerSkeletalMeshProperty, FishingPoleSocketNameProperty)*/FSimpleDelegate::CreateLambda([this, OwnerSkeletalMeshProperty, FishingPoleSocketNameProperty, CarryFishSocketNameProperty]()
    {
...
/*FText FDetailCustomization_FishingComponent::OnTextContentChanged() const
{
	FName CurrentValue;
	SocketOrBoneNameProperty->GetValue(CurrentValue);
	return FText::FromName(CurrentValue);
}

TSharedRef<SWidget> FDetailCustomization_FishingComponent::OnGenerateWidget(TSharedPtr<FName> Item)
{
	return SNew(STextBlock).Text(FText::FromName(*Item));
}*/
